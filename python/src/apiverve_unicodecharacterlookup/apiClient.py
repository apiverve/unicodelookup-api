...
class UnicodelookupAPIClientError(Exception):
...
class ValidationError(UnicodelookupAPIClientError):
...
class UnicodelookupAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"character": {"type": "string", "required": False}, "codepoint": {"type": "string", "required": False}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the UnicodelookupAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises UnicodelookupAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/unicodelookup'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise UnicodelookupAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[UnicodelookupAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns UnicodeCharacterLookupData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises UnicodelookupAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise UnicodelookupAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise UnicodelookupAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise UnicodelookupAPIClientError(f"Request failed: {str(e)}")
